...
柱的另一端受到幅值为1.0 N/m^2的简谐压力脉冲激励。在Abaqus/Standard中对0到100 Hz范围内的频率执行稳态动态分析。瞬态模拟也在Abaqus/Explicit中使用100 Hz的激励频率执行。可以通过更改输入文件中定义的参数来测试不同的激励频率。解应表示在正*y*方向上移动的稳态无衰减波。不会产生共振频率；整个柱中的最大压力应始终保持在1.0 N/m^2的幅值，相位应在波长的距离上下降2![](../graphics/ver_eqn00273.gif)弧度，其中*f*是以周期/时间为单位的激励频率。
