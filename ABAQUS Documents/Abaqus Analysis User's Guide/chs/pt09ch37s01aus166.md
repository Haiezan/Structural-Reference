...
可以通过几何缩放默认接触刚度来构建替代分段线性表格压力-过闭合关系。此模型提供了一个简单的界面，可在超过临界穿透时增加默认接触刚度。穿透度量![](../graphics/usb_eqn00870.gif)可以直接定义，也可以定义为接触区域中最小单元长度的分数，![](../graphics/usb_eqn00855.gif)。每次当前穿透超过此穿透度量的倍数时，接触刚度按因子![](../graphics/usb_eqn06198.gif)![](../graphics/usb_eqn01062.gif)缩放（参见图37.1.2-3）。初始刚度设置为默认接触刚度![](../graphics/usb_eqn06199.gif)乘以因子![](../graphics/usb_eqn00159.gif)。
