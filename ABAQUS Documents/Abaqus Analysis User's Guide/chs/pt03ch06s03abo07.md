...
| **Abaqus/CAE用法：** | 分析步模块：****步骤**![](../graphics/images/arrow.gif)**创建****：**频率**：**使用基于SIM的线性动力学过程** |
| --- | --- |
...
| --- | --- |
|  | 分析步模块：****步骤**![](../graphics/images/arrow.gif)**创建****：**频率**：**特征求解器：**Lanczos**，**使用基于SIM的线性动力学过程**，切换**投影阻尼算子** 控制使用AMS特征求解器的频率提取步骤中单元和材料阻尼的投影：分析步模块：****步骤**![](../graphics/images/arrow.gif)**创建****：**频率**：**特征求解器：**AMS**，切换**投影阻尼算子** |
...
其中![](../graphics/usb_eqn00533.gif)表示如阻尼器等单元的粘性阻尼矩阵。在基于模态的过程中，投影![](../graphics/usb_eqn00528.gif)到特征模态导致非对角矩阵。
...
您可以分别提供全局质量和刚度比例粘性阻尼因子，![](../graphics/usb_eqn00536.gif)和![](../graphics/usb_eqn00537.gif)，以使用全局模型质量和刚度矩阵分别创建全局阻尼矩阵，![](../graphics/usb_eqn00538.gif)和![](../graphics/usb_eqn00539.gif)：
...
其中下标*M*指模态号，![](../graphics/usb_eqn00546.gif)、![](../graphics/usb_eqn00547.gif)和![](../graphics/usb_eqn00548.gif)是与第M个模态相关的阻尼、质量和刚度项。
...
| --- | --- |
|  | 分析步模块：**创建步骤**：**线性扰动**：*任何有效步骤类型*：**阻尼**：**在以下范围内指定阻尼：模态**，**Rayleigh**：**使用Rayleigh阻尼数据** 使用以下输入通过指定频率范围定义Rayleigh阻尼：分析步模块：**创建步骤**：**线性扰动**：*任何有效步骤类型*：**阻尼**：**在以下范围内指定阻尼：频率**，**Rayleigh**：**使用Rayleigh阻尼数据** |
...
其中*m*是系统的质量，*k*是系统的刚度。临界阻尼分数，![](../graphics/usb_eqn00550.gif)的典型值在临界阻尼的1%到10%之间，![](../graphics/usb_eqn00551.gif)；但Abaqus/Standard接受任何正值。临界阻尼因子可以从步骤到步骤更改。
...
| --- | --- |
|  | 分析步模块：**创建步骤**：**线性扰动**：*任何有效步骤类型*：**阻尼**：**在以下范围内指定阻尼：模态**，**Direct modal**：**使用直接阻尼数据** 使用以下输入通过指定频率范围定义临界阻尼分数：分析步模块：**创建步骤**：**线性扰动**：*任何有效步骤类型*：**阻尼**：**在以下范围内指定阻尼：频率**，**Direct modal**：**使用直接阻尼数据** |
...
其中![](../graphics/usb_eqn00554.gif)表示材料结构性阻尼，![](../graphics/usb_eqn00555.gif)表示如具有复刚度的弹簧和连接器等单元的结构性阻尼系数，![](../graphics/usb_eqn00556.gif)是实单元刚度矩阵。在基于模态的过程中，投影![](../graphics/usb_eqn00557.gif)到模态形状导致完整模态阻尼矩阵。当使用基于SIM的模态过程时，投影的材料和单元阻尼矩阵可以与全局和模态阻尼组合（请参见下面的["定义和使用全局和模态对角阻尼"](pt03ch06s03abo07.md#usb-anl-adynamicproc-using)）。材料/单元结构性阻尼不适用于声学单元。
...
| --- | --- |
|  | 分析步模块：**创建步骤**：**线性扰动**：*任何有效步骤类型*：**阻尼**：**在以下范围内指定阻尼：模态**，**Structural**：**使用结构性阻尼数据** 使用以下输入通过指定频率范围定义结构性阻尼：分析步模块：**创建步骤**：**线性扰动**：*任何有效步骤类型*：**阻尼**：**在以下范围内指定阻尼：频率**，**Structural**：**使用结构性阻尼数据** |
...
如果指定复合模态阻尼，Abaqus会在特征频率提取步骤中从您为每个材料定义的阻尼因子计算阻尼系数![](../graphics/usb_eqn00564.gif)。

| **输入文件用法：** | 使用以下两个选项： |
| --- | --- |
|  | ``` [*DAMPING](../key/key-link.md#usb-kws-mdamping), COMPOSITE=![](../graphics/usb_eqn00565.gif)![](../graphics/usb_eqn00565.gif) [*MODAL DAMPING](../key/key-link.md#usb-kws-hmodaldamp), VISCOUS=COMPOSITE ``` |

| **Abaqus/CAE用法：** | 属性模块：材料编辑器：****Mechanical**![](../graphics/images/arrow.gif)**Damping****：**Composite**：![](../graphics/usb_eqn00565.gif) 分析步模块：**创建步骤**：**线性扰动**：*任何有效步骤类型*：**阻尼**：**Composite modal**：**使用复合阻尼数据** |
| --- | --- |
