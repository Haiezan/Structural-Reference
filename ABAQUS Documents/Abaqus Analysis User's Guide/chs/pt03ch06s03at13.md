...
| **Abaqus/CAE用法：** | 分析步模块：**创建步骤**：**线性扰动**：**稳态动力学，模态** |
| --- | --- |
...
| **Abaqus/CAE用法：** | 分析步模块：**创建步骤**：**线性扰动**：**稳态动力学，模态**：**使用特征频率细分每个频率范围** |
| --- | --- |
...
| **Abaqus/CAE用法：** | 分析步模块：**创建步骤**：**线性扰动**：**稳态动力学，模态**：关闭**使用特征频率细分每个频率范围** |
| --- | --- |
...
| **Abaqus/CAE用法：** | 分析步模块：**创建步骤**：**线性扰动**：**稳态动力学，模态**：**标度：对数**或**线性** |
| --- | --- |
...
| **Abaqus/CAE用法：** | 分析步模块：**创建步骤**：**线性扰动**：**稳态动力学，模态**：**数据**：在表格中输入数据，并根据需要添加行 |
| --- | --- |
...
对于稳态分析，几乎总是指定阻尼（见["材料阻尼，" 第26.1.1节"](pt05ch26s01abm51.md)）。如果没有阻尼，当激励频率等于结构的特征频率时，结构的响应将是无界的。要获得定量的准确结果，特别是在固有频率附近，准确指定阻尼特性至关重要。各种阻尼选项在["材料阻尼，" 第26.1.1节"](pt05ch26s01abm51.md)中讨论。您可以为用于响应计算的所有或部分模态定义阻尼系数。阻尼系数可以为指定模态号或指定频率范围给出。当通过指定频率范围定义阻尼时，模态的阻尼系数在指定频率之间线性插值。频率范围可以是不连续的；在不连续处的特征频率将应用平均阻尼值。阻尼系数在指定频率范围之外被认为是常数。
...
| --- | --- |
|  | 分析步模块：**创建步骤**：**线性扰动**：**稳态动力学，模态**：**阻尼** 在Abaqus/CAE中不支持通过指定频率范围来定义阻尼。 |
...
基态是稳态动态步骤之前最后一个常规分析步骤结束时模型的当前状态。如果分析的第一个步骤是扰动步骤，则基态由初始条件决定（["Abaqus/Standard和Abaqus/Explicit中的初始条件，" 第34.2.1节"](pt07ch34s02aus116.md)）。直接定义求解变量（如速度）的初始条件定义不能在稳态动态分析中使用。
