...
- 在具有各向异性剪切行为的三维问题的最一般情况下，需要几个"数据块"。如前所述，每个"数据块"将包含在固定值下![](../graphics/usb_eqn05580.gif)对![](../graphics/usb_eqn05608.gif)。在每个"数据块"中，![](../graphics/usb_eqn05609.gif)![](../graphics/usb_eqn05608.gif)可以在0到1.0之间变化。情况![](../graphics/usb_eqn05615.gif)（任何"数据块"中的第一个数据点）对应于纯法向模式，当![](../graphics/usb_eqn05616.gif)时（即图37.1.10-6中线OB上唯一有效的点是O，对应于纯法向分离）永远无法实现。然而，在将断裂能定义为模式混合函数的表格定义中，这个点简单地用于设置一个极限，确保当从各种法向和剪切分离组合接近纯法向状态时，断裂能发生连续变化。因此，每个"数据块"中第一个数据点的断裂能必须始终设置为纯法向分离时的断裂能（![](../graphics/usb_eqn05599.gif)）。作为各向异性剪切情况的示例，假设您要输入三个"数据块"，对应于固定的![](../graphics/usb_eqn05617.gif)值分别为0.、0.2和1.0。对于三个"数据块"中的每一个，根据上面讨论的原因，第一个数据点必须是![](../graphics/usb_eqn05618.gif)。每个"数据块"中的其余数据点定义了断裂能随剪切分离比例增加的变化。

##### 基于牵引的模式混合

断裂能需要以![](../graphics/usb_eqn05580.gif)对![](../graphics/usb_eqn05619.gif)和![](../graphics/usb_eqn05620.gif)的表格形式指定。因此，![](../graphics/usb_eqn05580.gif)需要在各种固定的值下定义为![](../graphics/usb_eqn05619.gif)的函数。在这种情况下，"数据块"对应于在固定![](../graphics/usb_eqn05620.gif)值下![](../graphics/usb_eqn05580.gif)对![](../graphics/usb_eqn05619.gif)的数据集。在每个"数据块"中，![](../graphics/usb_eqn05620.gif)![](../graphics/usb_eqn05619.gif)可以从0（纯法向分离）到1（纯剪切分离）。一个重要限制是，每个数据块必须为![](../graphics/usb_eqn05621.gif)指定相同的断裂能值。这个限制确保了当牵引向量接近法向方向时，失效所需的能量不依赖于牵引向量在剪切平面上的投影方向（参见图37.1.10-2）。
