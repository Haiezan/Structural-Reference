...
接触压力分布在许多 Abaqus 分析中是关键关注点。您可以查看除解析刚体表面和基于刚性类型单元的离散刚体表面外的所有接触表面上的接触压力（后者限制不适用于通用接触）。您可以请求接触压力误差指标的轮廓图以及接触压力的轮廓图，以在接触压力解决方案重要的区域获得接触压力解决方案局部精度的视角（见 ["选择影响自适应重划分网格的误差指标，" 第 12.3.2 节](pt04ch12s03aus84.md)，了解关于误差指标输出的进一步讨论）。
