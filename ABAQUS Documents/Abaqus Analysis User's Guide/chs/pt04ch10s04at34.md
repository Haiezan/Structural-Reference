...
循环对称结构的非对称响应提取目前仅适用于使用块 Lanczos 方法的特征频率提取分析（["固有频率提取，" 第 6.3.5 节"](pt03ch06s03at10.md)）和频域、基于模态的稳态动力学分析（["基于模态的稳态动力学分析，" 第 6.3.8 节"](pt03ch06s03at13.md)）。可以为特定循环对称模式、一组循环对称模式或所有循环对称模式提取对应于对称和非对称特征模态的固有频率。它们可以在后续稳态动力学分析中使用。解被投影到的特征模态的选择如["基于模态的稳态动力学分析"第 6.3.8 节"](pt03ch06s03at13.md#usb-anl-asteadystdyn-selecteigen-damping)中的"选择模态和指定阻尼"所述。
...
可以使用具有 Lanczos 特征求解器的特征频率提取过程来提取循环对称结构的固有频率和相应特征模态（请参阅["固有频率提取，" 第 6.3.5 节"](pt03ch06s03at10.md)）。特征频率提取过程不需要其他信息。所有固有频率以常规（升序）排序。对于每个固有频率，报告循环对称模式编号。

特征模态以与数据（`.dat`）、结果（`.fil`）和输出数据库（`.odb`）文件中仅针对用户指定的基准扇区写入的固有频率对应的顺序写入。这些模态可以根据循环对称模式编号在 Abaqus/CAE 中扩展到整个结构。

有两种不同类型的特征模态：单个和配对。0 倍循环对称的特征模态始终是单个的。对于偶数 *N*，![](../graphics/usb_eqn01602.gif) 倍循环对称的特征模态也是单个的。其余 ![](../graphics/usb_eqn01603.gif)（偶数 *N*）或 ![](../graphics/usb_eqn01604.gif)（奇数 *N*）循环对称模式的特征模态是配对的。配对特征模态对应的固有频率相等，并且总是在数据文件中的固有频率表中一起出现。将具有 *k* 倍循环对称性的特征模态 ![](../graphics/usb_eqn01605.gif) 扩展到扇区 ![](../graphics/usb_eqn01606.gif) 可以通过以下方式进行：
...
这里 ![](../graphics/usb_eqn01609.gif) 和 ![](../graphics/usb_eqn01610.gif) 分别是第一（基准）扇区和第 *i* 扇区上双固有频率对应的配对特征模态；以及 ![](../graphics/usb_eqn01611.gif)。
...
在基于模态的稳态动力学分析中，载荷为特定循环对称模式在基准扇区上定义，这在载荷定义中指明。对于 *k* 倍循环对称模式 ![](../graphics/usb_eqn01615.gif)，扇区  上的复载荷 ![](../graphics/usb_eqn01616.gif) 和 ![](../graphics/usb_eqn01617.gif)（分别对应于实部和虚部分量）通过以下方式获得：

![](../graphics/usb_eqn01618.gif)![](../graphics/usb_eqn01619.gif)
...
一般过程的循环对称模型对材料模型没有特定限制。对于频率分析过程，请参阅["固有频率提取，" 第 6.3.5 节"](pt03ch06s03at10.md)中的说明。
...
在特征值提取过程中，以下特殊条件适用：
- 如果选择位移特征向量归一化（默认），则基准扇区上每个特征向量中的最大位移条目为单位 1。如果选择质量特征向量归一化，则对特征向量进行归一化，使基准扇区上计算的广义质量为单位 1。详情请参阅["固有频率提取，" 第 6.3.5 节"](pt03ch06s03at10.md)。
