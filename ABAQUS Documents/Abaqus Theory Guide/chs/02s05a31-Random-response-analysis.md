...
现在让我们考虑相反的情况，称为"窄带"函数。这种函数最极端的情况是在单一频率的简单正弦振动：![](../graphics/stm_eqn01428.gif)。那么 ![](../graphics/stm_eqn01423.gif) 也必须是周期性的，因为 ![](../graphics/stm_eqn01423.gif) 必须每次在平移  对应于振动周期时达到相同的值 ![](../graphics/stm_eqn01429.gif)。通过对时间进行积分，

![](../graphics/stm_eqn01399.gif)![](../graphics/stm_eqn01430.gif)
...
因此，自相关 ![](../graphics/stm_eqn01423.gif)![](../graphics/stm_eqn01423.gif) 告诉我们随机变量的性质。如果当时间平移 ![](../graphics/stm_eqn01399.gif) 远离 ![](../graphics/stm_eqn01424.gif) 时 ![](../graphics/stm_eqn01423.gif) 迅速衰减，则变量具有宽频率内容；如果衰减较慢并呈现余弦轮廓，则变量具有以 ![](../graphics/stmnarrow-band-nls.png)

我们可以通过将变量与正弦波互相关来将此概念扩展到检测随机变量的频率内容：扫描一系列频率的波并检查互相关告诉我们随机变量是否由特定频率的振荡主导。我们开始看到，平稳、遍历随机过程的性质最好通过在频域中检查来理解。

作为说明，考虑一个包含许多离散频率的变量 ![](../graphics/stm_eqn01388.gif)![](../graphics/stm_eqn01388.gif)。我们可以将其写为以基本频率 ![](../graphics/stm_eqn01433.gif) 的 *N* 步展开的傅里叶级数：
...
因此，在频率 ![](../graphics/stm_eqn01447.gif)![](../graphics/stm_eqn01448.gif) 处对 *x* 方差的贡献，每单位频率为
...
![](../graphics/stm_eqn01498.gif)其中 ![](../graphics/stm_eqn01499.gif) 是频率  处的 ![](../graphics/stm_eqn01500.gif)，*N* 是计算响应的点数。*N* 将取决于叠加中使用的特征模态数量和用户在特征频率之间指定的点数。
...
![](../graphics/stm_eqn01501.gif)![](../graphics/stm_eqn01502.gif)其中 ![](../graphics/stm_eqn01186.gif) 是质量矩阵，![](../graphics/stm_eqn01265.gif) 是阻尼矩阵，![](../graphics/stm_eqn01503.gif) 是刚度矩阵，![](../graphics/stm_eqn00694.gif) 是外部载荷，![](../graphics/stm_eqn00657.gif) 是有限元模型自由度 *N* 的值（通常是位移或旋转分量，或声压），![](../graphics/stm_eqn01504.gif) 是任意的虚位移。
...
![](../graphics/stm_eqn01562.gif)给出。类似地，节点 a 处 von Mises 应力的 RMS 计算为
