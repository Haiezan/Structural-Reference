...
Abaqus中的单元库包含几种类型的梁单元。此上下文中的"梁"是进行了假设以将问题数学上简化为一维的单元：主求解变量仅是沿梁轴线位置位置的函数。为了使这些假设合理，直觉上很清楚，梁必须是一个连续体，我们可以在其中定义一条轴线，使得从该轴线到连续体中任何点的最短距离与沿轴线的典型长度相比较小。这个概念在"梁单元公式，" 第3.5.2节中的详细推导中变得更加精确。将问题简化为一维问题的假设有几种复杂程度，Abaqus中不同的梁单元使用不同的假设。
