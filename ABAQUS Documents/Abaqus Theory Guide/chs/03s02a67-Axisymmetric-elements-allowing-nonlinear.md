...
Abaqus/Standard包含一组实体单元，其几何形状最初是轴对称的，允许进行可以发生关于模型柱面坐标系（*r*、*z*、）中平面![](../graphics/stm_eqn03117.gif)弯曲的非线性分析。几何模型仅在*r*—*z*平面中定义。位移是相对于*r*和*z*的常规参数插值，并通过对![](../graphics/stm_eqn01111.gif)![](../graphics/stm_eqn01111.gif)的Fourier展开来增强。因为单元编写为仅允许关于平面![](../graphics/stm_eqn03117.gif)弯曲，它们不能用于模拟关于原始对称轴的结构扭转。因为单元用于非线性应用，与Fourier模式相关的正交性质不能用于将问题简化为更小的、不耦合的情况，因为投影到Fourier模式之前的刚度不一定是恒定的。由于这个原因，这些单元比用于轴对称变形的相应轴对称单元要昂贵得多。
### 插值
...
使用相对于的Fourier项的![](../graphics/stm_eqn00428.gif)的一般插值方案：

![](../graphics/stm_eqn01111.gif)![](../graphics/stm_eqn03124.gif)其中*g*、*h*是原始*R*—*Z*平面中的参数坐标；![](../graphics/stm_eqn03125.gif)是多项式插值函数；![](../graphics/stm_eqn03126.gif)、![](../graphics/stm_eqn03127.gif)和![](../graphics/stm_eqn03128.gif)是解振幅值。*M*是用于相对于*g*、*h*插值的项数；*P*是用于相对于![](../graphics/stm_eqn01111.gif)的Fourier插值的项数。当![](../graphics/stm_eqn03129.gif)时，产生纯轴对称变形。

我们通过假定弯曲仅允许关于一个平面![](../graphics/stm_eqn03117.gif)来减少此类单元中的变量数，因此平面![](../graphics/stm_eqn03130.gif)（*n*整数）是对称平面。满足此条件的唯一项是

![](../graphics/stm_eqn03131.gif)为方便起见，我们使用在模型中在之间特定位置的![](../graphics/stm_eqn03072.gif)和![](../graphics/stm_eqn03056.gif)位移分量的值，而不是Fourier振幅![](../graphics/stm_eqn03132.gif)![](../graphics/stm_eqn03133.gif)![](../graphics/stm_eqn03134.gif)和![](../graphics/stm_eqn03135.gif)。主要原因是允许单元与界面单元（如滑移线）一起使用，界面单元需要物理位移值。只有在假定相对于![](../graphics/stm_eqn01111.gif)方向的相对位移很小，使得界面条件相对于![](../graphics/stm_eqn03055.gif)和![](../graphics/stm_eqn03056.gif)考虑时（即，在常数![](../graphics/stm_eqn01111.gif)的平面中），这才是准确的。此外，我们在周向位移表达式中省略下标*s*：![](../graphics/stm_eqn03136.gif)。因此，[公式3.2.9-1](03s02a67-Axisymmetric-elements-allowing-nonlinear.md)被重写为
