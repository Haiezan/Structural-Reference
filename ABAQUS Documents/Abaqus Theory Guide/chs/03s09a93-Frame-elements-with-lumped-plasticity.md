...
框架单元根据用户定义的端节点处的求解变量和与内部节点相关的额外内部自由度来公式化。这里讨论单元的三维版本。二维版本通过对三维自由度的适当简化获得。单元有三个节点（两个用户定义和一个内部），12个外部自由度和三个内部自由度。两个端节点中的每一个都有六个外部自由度：三个位移和三个旋转。一个内部节点（在单元中心）只有三个位移自由度，如图3.9.2-1所示。
...
![](../graphics/stm_eqn04525.gif)![](../graphics/stm_eqn05179.gif)
...
因此，弹性刚度是一个15×15矩阵，将力向量![](../graphics/stm_eqn05184.gif)![](../graphics/stm_eqn05185.gif)与节点位移向量![](../graphics/stm_eqn02599.gif)关联：
...
![](../graphics/stm_eqn05192.gif)其中![](../graphics/stm_eqn00424.gif)表示在节点温度处评估的![](../graphics/stm_eqn05193.gif)材料矩阵：

![](../graphics/stm_eqn01111.gif)![](../graphics/stm_eqn05194.gif)*A*是横截面积；*E*是杨氏模量；*G*是剪切模量；![](../graphics/stm_eqn03733.gif)、![](../graphics/stm_eqn05195.gif)和![](../graphics/stm_eqn03734.gif)是横截面惯性矩；![](../graphics/stm_eqn05196.gif)是扭转刚度。广义初始力向量包括以下分量：
...
定义单元局部方向从参考值![](../graphics/stm_eqn05253.gif)到当前值![](../graphics/stm_eqn05254.gif)的旋转。

为定义应变感生旋转贡献，我们将每个节点的旋转乘性分解，

![](../graphics/stm_eqn05255.gif)![](../graphics/stm_eqn05256.gif)![](../graphics/stm_eqn05257.gif)其中![](../graphics/stm_eqn05258.gif)是节点旋转的应变感生部分。由于假定应变很小，在节点*I*处将Euler-Bernoulli旋转定义为轴向量![](../graphics/stm_eqn05259.gif)，使得
