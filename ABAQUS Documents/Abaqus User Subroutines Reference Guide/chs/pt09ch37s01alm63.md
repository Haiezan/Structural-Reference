...
您可以指定数量![](../graphics/usb_eqn05581.gif)（即，相对于损伤起始时的有效分离的完全失效时的有效分离![](../graphics/usb_eqn05582.gif)，如图37.1.10-3所示，作为模式混合、温度和/或场变量的表格函数。此外，您还可以选择线性或指数软化定律，定义损伤变量*D*在损伤起始后和完全失效之间的详细演化（作为超过损伤起始的有效分离的函数）。或者，与使用线性或指数软化不同，您可以直接将损伤变量*D*指定为损伤起始后有效分离相对于损伤起始有效分离的表格函数，![](../graphics/usb_eqn05583.gif)![](../graphics/usb_eqn05584.gif)；模式混合；温度；和/或场变量。
...
纯法向和纯第一和第二剪切方向分离的极限情况在图37.1.10-6中分别由![](../graphics/usb_eqn05599.gif)、![](../graphics/usb_eqn05600.gif)和![](../graphics/usb_eqn05612.gif)表示。标记为"Modes n-s"、"Modes n-t"和"Modes s-t"的线分别显示了在纯法向和纯第一方向剪切之间、纯法向和纯第二方向剪切之间、以及纯第一和纯第二方向剪切之间的行为转变。一般来说，![](../graphics/usb_eqn05580.gif)必须在各种固定值下定义为![](../graphics/usb_eqn05608.gif)的函数。在下面的讨论中，我们将相对于对应于固定![](../graphics/usb_eqn05609.gif)的![](../graphics/usb_eqn05580.gif)![](../graphics/usb_eqn05608.gif)的数据集称为"数据块"。以下指南在将断裂能定义为模式混合的函数时很有用：
- 对于二维问题，![](../graphics/usb_eqn05609.gif)![](../graphics/usb_eqn05580.gif)只需要定义为![](../graphics/usb_eqn04768.gif)（在这种情况下为![](../graphics/usb_eqn05613.gif)）的函数。对应于![](../graphics/usb_eqn05609.gif)的数据列必须留空。因此，基本上只需要一个"数据块"。
...
- 在具有各向同性剪切行为的三维问题的最一般情况下，需要几个"数据块"。如前所述，每个"数据块"将包含在固定值下![](../graphics/usb_eqn05580.gif)相对于![](../graphics/usb_eqn05608.gif)。在每个"数据块"中，![](../graphics/usb_eqn05609.gif)![](../graphics/usb_eqn05608.gif)可以在0到1.0之间变化。情况![](../graphics/usb_eqn05615.gif)（任何"数据块"中的第一个数据点）对应于纯法向模式，当![](../graphics/usb_eqn05616.gif)时（即，在图37.1.10-6中线OB上唯一有效的点是O，对应于纯法向分离）永远无法实现。但是，在将断裂能作为模式混合函数表格定义中，此点简单地用于设置极限，确保当从各种法向和剪切实分组合接近纯法向状态时，断裂能发生连续变化。因此，每个"数据块"中第一个数据点的断裂能必须始终设置为等于纯法向分离中的断裂能（![](../graphics/usb_eqn05599.gif)）。作为各向同性剪切情况的示例，假设您要输入三个"数据块"，分别对应于固定值![](../graphics/usb_eqn05617.gif)0.、0.2和1.0。对于三个"数据块"中的每一个，根据上面讨论的原因，第一个数据点必须是![](../graphics/usb_eqn05618.gif)。每个"数据块"中的其余数据点定义了断裂能随剪切分离比例增加的变化。

##### 基于牵引的模式混合

断裂能需要以![](../graphics/usb_eqn05580.gif)相对于![](../graphics/usb_eqn05619.gif)和![](../graphics/usb_eqn05620.gif)的表格形式指定。因此，![](../graphics/usb_eqn05580.gif)需要在各种固定值下指定为![](../graphics/usb_eqn05619.gif)的函数。在这种情况下，"数据块"对应于在固定值![](../graphics/usb_eqn05620.gif)下![](../graphics/usb_eqn05580.gif)相对于![](../graphics/usb_eqn05619.gif)的数据集。在每个"数据块"中，![](../graphics/usb_eqn05620.gif)![](../graphics/usb_eqn05619.gif)可以从0（纯法向分离）到1（纯剪切分离）变化。一个重要的限制是，每个数据块必须为![](../graphics/usb_eqn05621.gif)指定相同的断裂能值。此限制确保当牵引向量接近法向方向时，断裂所需的能量不取决于牵引向量在剪切平面上的投影方向（见图37.1.10-2）。
