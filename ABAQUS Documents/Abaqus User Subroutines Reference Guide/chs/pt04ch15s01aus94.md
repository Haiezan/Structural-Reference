...
在许多情况下，仔细设置接触刚度很重要；例如，允许 DEM 结果匹配变形球体之间的 Hertz 接触行为。如果您指定了 DEM 接触刚度，则必须确保用于分析的时间增量足够小以避免数值不稳定。对于每个粒子同时接触许多粒子的密集三维颗粒堆积，数值稳定性考虑很复杂。一般准则是不应超过时间增量，其中 *m* 和 *k* 分别代表粒子质量和接触刚度。在某些应用中，甚至更小的时间增量（如 ![](../graphics/usb_eqn02110.gif)![](../graphics/usb_eqn02111.gif)）可能导致改进的解决方案。
