...
其中*p*是流体中的动态压力（超过任何初始静压力的压力），![](../graphics/usb_eqn04228.gif)是流体粒子的空间位置，![](../graphics/usb_eqn04229.gif)是流体粒子速度，![](../graphics/usb_eqn04230.gif)是流体粒子加速度，![](../graphics/usb_eqn01247.gif)是流体密度，![](../graphics/usb_eqn00060.gif)是由流体流过矩阵材料引起的"体积拖曳"（每单位体积每速度的力）。d'Alembert项是在假设流体没有稳态流动的情况下不考虑对流写的，这通常被认为对于高达马赫0.1的稳态流体速度足够准确。

流体的本构行为被认为是无粘性和可压缩的，因此声学介质的体积模量通过以下方式将介质中的动态压力与体积应变关联

![](../graphics/usb_eqn04231.gif)

其中![](../graphics/usb_eqn04232.gif)是体积应变。必须定义声学介质的体积模量![](../graphics/usb_eqn04233.gif)和密度![](../graphics/usb_eqn01247.gif)。

体积模量![](../graphics/usb_eqn04233.gif)可以定义为温度和场变量的函数，但在使用子空间投影方法的隐式动态分析（["使用直接积分的隐式动态分析，" 第6.3.2节](pt03ch06s03at07.md)）或直接解稳态动态分析（["直接解稳态动态分析，" 第6.3.4节](pt03ch06s03at09.md)）期间不变化值；对于这些程序，使用步开始时体积模量的值。
...
在频域程序中，![](../graphics/usb_eqn00060.gif)可能依赖于频率。![](../graphics/usb_eqn00060.gif)可以作为频率的函数输入——![](../graphics/usb_eqn04234.gif)，其中*f*是每时间周期的频率（通常为Hz）——除了温度和/或场变量外，仅当声学介质用于稳态动力学程序时。如果声学介质用于直接积分动态程序（包括Abaqus/Explicit），则体积拖曳系数被认为与频率无关，并使用为当前温度和/或场变量输入的第一个值。

在除直接稳态动力学外的所有程序中，![](../graphics/usb_eqn01246.gif)的梯度被认为很小。
...
其中![](../graphics/usb_eqn02074.gif)是实值电阻率，![](../graphics/usb_eqn00562.gif)是实值无量纲孔隙率，![](../graphics/usb_eqn02795.gif)是无量纲"结构因子"，![](../graphics/usb_eqn04236.gif)是无量纲波数。这个方程可以重写为
...
Abaqus/Standard支持[Delany & Bazley (1970)](pt05ch26s03abm58.md#delany)提出的著名经验模型，该模型将材料属性确定为频率和用户定义的流动电阻率，![](../graphics/usb_eqn02074.gif)；密度，![](../graphics/usb_eqn01247.gif)；和体积模量，![](../graphics/usb_eqn01253.gif)的函数。[Miki (1990)](pt05ch26s03abm58.md#miki)提出的该模型的变体也可用。这些模型仅支持稳态动力学程序。

两种模型都根据以下公式计算依赖于频率的材料特性阻抗，![](../graphics/usb_eqn04241.gif)，和波数或传播常数，![](../graphics/usb_eqn04242.gif)![](../graphics/usb_eqn04243.gif)
...
这些参数的符号必须与Abaqus中使用的符号约定和能量守恒一致。Abaus使用傅里叶变换对形式上等同于假设![](../graphics/usb_eqn04253.gif)时间依赖。因此，密度和体积模量的实部对所有频率值都是正的，体积模量的虚部必须为正，密度的虚部必须为负。

线性各向同性声学材料可以用两个频率依赖参数完全描述：体积模量，![](../graphics/usb_eqn04254.gif)，和密度，![](../graphics/usb_eqn04255.gif)。然而，通常会遇到以其他参数对定义的材料，如特性阻抗，![](../graphics/usb_eqn04241.gif)，波数或传播常数，![](../graphics/usb_eqn04242.gif)，或声速，![](../graphics/usb_eqn04256.gif)。使用对![](../graphics/usb_eqn04257.gif)或![](../graphics/usb_eqn04258.gif)定义的数据可以转换为复密度和体积模量形式，从以下标准公式开始：
...
与Abaqus符号约定一致，![](../graphics/usb_eqn04242.gif)和![](../graphics/usb_eqn04256.gif)的实部必须为正；![](../graphics/usb_eqn04242.gif)的虚部必须为负，![](../graphics/usb_eqn04256.gif)的虚部必须为正。在通常观察到的材料中，这些常数中虚部与实部的大小的比值通常远小于1。
...
![](../graphics/usb_eqn04254.gif)的实部和虚部分别为：
...
其中![](../graphics/usb_eqn01799.gif)是流体空化极限，![](../graphics/usb_eqn02314.gif)是初始声学静压力。对于发生空化的非线性声学介质，使用总波 formulations。这 formulation与散射波formulations非常相似，只是伪压力（定义为体积模量与压缩体积应变的乘积）起着材料状态变量的作用，而不是声学动态压力，声学动态压力可以容易地从这个伪压力在空化条件下获得。
