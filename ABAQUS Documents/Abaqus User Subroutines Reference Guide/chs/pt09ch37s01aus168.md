...
其中![](../graphics/usb_eqn06212.gif)是单元阻塞面积，![](../graphics/usb_eqn04609.gif)是单元面积，![](../graphics/usb_eqn01422.gif)是单元节点的总数，![](../graphics/usb_eqn05808.gif)是单元中接触堵塞活跃节点的总数。当所有单元节点对接触堵塞活跃时，单元被接触表面完全阻塞。总阻塞面积是所有单元阻塞面积的总和。泄漏面积用于流体交换计算，方法是从表面总面积中减去总阻塞面积（如果未为流体交换指定有效面积）。如果同时指定了有效面积和表面（见["流体交换定义，" 第11.5.3节"](pt04ch11s05aus72.md)），则用于流体交换计算的泄漏面积是通过使用总阻塞面积与表面总面积的比率乘以有效面积获得的。在这种情况下，可以使用基于节点的表面，泄漏面积是通过使用接触堵塞活跃节点总数与表面中定义的节点总数的比率获得的。
