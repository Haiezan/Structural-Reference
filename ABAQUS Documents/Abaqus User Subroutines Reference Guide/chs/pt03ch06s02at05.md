...
当不存在稳定的循环响应时，方法将不会收敛。在发生塑性棘轮的情况下，Fourier级数中所有周期项（![](../graphics/usb_eqn00477.gif)和![](../graphics/usb_eqn00478.gif)）的位移和残差系数会收敛。但是，Fourier级数中常数项（![](../graphics/usb_eqn00479.gif)和![](../graphics/usb_eqn00469.gif)）的位移和残差系数从一个迭代继续增长到另一个迭代。用户指定的容差![](../graphics/usb_eqn00480.gif)和![](../graphics/usb_eqn00481.gif)用于检测塑性棘轮。默认值为![](../graphics/usb_eqn00480.gif) = 0.005和![](../graphics/usb_eqn00481.gif) = 0.005。有关更多信息，请参见["常用控制参数，" 第7.2.2节中的"控制直接循环分析中的求解精度"](pt03ch07s02aus50.md#usb-anl-aconvergecontrol-directcyclic)。
...
其中*t*是增量开始时的时间，![](../graphics/usb_eqn00456.gif)是时间增量（因此![](../graphics/usb_eqn00457.gif)是增量结束的时间），![](../graphics/usb_eqn00458.gif)是等效蠕变应变率。为达到足够精度，对于蠕变问题，精度容差参数的值应选择在![](../graphics/usb_eqn00459.gif)的数量级，其中![](../graphics/usb_eqn00460.gif)是应力中可接受的误差水平，*E*是典型弹性模量，或者对于粘弹性问题，在弹性应变的数量级。

如果率相关本构方程与变化温度结合使用，两种控制可以同时使用。然后Abaqus/Standard将选择满足两个标准的增量。

如果上述一个或两个控制指定的时间积分精度度量在连续![](../graphics/usb_eqn00485.gif)次增量后满足，则下一个时间增量将增加一个因子![](../graphics/usb_eqn00486.gif)。![](../graphics/usb_eqn00485.gif)和![](../graphics/usb_eqn00486.gif)是用户定义的参数（请参见["瞬态问题中的时间积分精度"中的"增加时间增量大小"，第7.2.4节"](pt03ch07s02aus52.md#usb-anl-aautomaticinc-increasedeltat)）。默认值为![](../graphics/usb_eqn00485.gif) = 3和![](../graphics/usb_eqn00486.gif) = 1.5。
