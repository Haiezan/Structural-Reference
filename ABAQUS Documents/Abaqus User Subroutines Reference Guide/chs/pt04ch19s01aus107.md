...
- 频率过程。对于频率步骤，*s*从涉及质量和刚度矩阵导数的矩阵的元素贡献计算得出（即![](../graphics/usb_eqn02144.gif)，其中![](../graphics/usb_eqn02145.gif)是刚度，![](../graphics/usb_eqn00668.gif)是质量，*h*是设计参数，![](../graphics/usb_eqn00445.gif)是特征值）。标量*s*被取为该矩阵到特征向量![](../graphics/usb_eqn02146.gif)上的投影。如果将扰动大小算法应用于具有不同特征值的模态，则![](../graphics/usb_eqn02146.gif)被取为与该模态关联的特征向量。然而，如果某个模态恰好与重特征值关联，则![](../graphics/usb_eqn02146.gif)被取为与重特征值关联的所有特征向量之和。因此，与重特征值关联的整个模态集将由扰动大小算法同时处理（重特征值的特征值灵敏度从相同的简化特征值系统同时获得）。
...
从上面的增量方程可以看出，显式设计依赖由两项组成。其中第一项![](../graphics/usb_eqn02150.gif)表示"直接设计依赖"，因为这项源于响应直接依赖于设计参数。第二个显式项![](../graphics/usb_eqn02151.gif)表示通过增量开始时的状态变量对设计参数的依赖。对于总量公式，可见显式项仅涉及直接设计依赖。

在Abaqus中实现了直接设计依赖计算的任何功能都将被称为DSA"支持"的功能。支持和不支持的功能可以在同一分析中混合使用，除非支持的功能导致不支持的功能变得直接依赖于设计（一个例子是使框架单元的杨氏模量依赖于设计，因为DSA不支持框架）。

为了更清楚地区分设计依赖的类型，考虑一个更具体的线性弹性桁架单元示例，一端固定，另一端受集中载荷拉伸。让![](../graphics/usb_eqn02152.gif)表示自由端的位移，*E*表示杨氏模量，*L*表示桁架的长度。将轴向应力![](../graphics/usb_eqn02153.gif)作为响应。虽然在这个简单的例子中，应力可以很容易地计算为载荷除以横截面积，但有限元分析计算应力的方式等价于![](../graphics/usb_eqn02154.gif)。选择杨氏模量*E*作为设计参数，应力灵敏度为
