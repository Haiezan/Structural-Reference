...
损伤萌生指材料点响应降解的开始。在低循环疲劳分析中，损伤萌生准则用每个循环的累积非弹性滞后能量，![](../graphics/usb_eqn00496.gif)来表征。![](../graphics/usb_eqn00496.gif)和材料常数用于确定损伤萌生的循环次数，![](../graphics/usb_eqn00497.gif)。在稳定载荷循环结束时，![](../graphics/usb_eqn00498.gif)，Abaqus/Standard检查任何材料点是否满足损伤萌生准则![](../graphics/usb_eqn00499.gif)；除非满足此准则，否则材料点的材料刚度不会被降解。与损伤萌生相关的计算和输出在["低循环疲劳中延性材料的损伤萌生，" 第24.4.2节"](pt05ch24s04abm48.md)中详细讨论。

一旦材料点满足损伤萌生准则，损伤状态基于稳定循环的非弹性滞后能量计算和更新。Abaqus/Standard假设弹性刚度的降解可以用标量损伤变量，![](../graphics/usb_eqn00500.gif)来建模。材料点每个循环的损伤率，![](../graphics/usb_eqn00501.gif)，基于累积非弹性滞后能量、与积分点相关的特征长度和材料常数计算。详细信息请参见["低循环疲劳中延性材料的损伤演化，" 第24.4.3节"](pt05ch24s04abm49.md)。

通常，当![](../graphics/usb_eqn00502.gif)时，材料完全失去其承载能力。如果单元所有积分位置的所有截面点都失去了其承载能力，您可以从网格中移除该单元。

#### 体材料中的损伤外推技术

如果在稳定循环结束时任何材料点满足损伤萌生准则，![](../graphics/usb_eqn00498.gif)，Abaqus/Standard将损伤变量，![](../graphics/usb_eqn00503.gif)从当前循环前向在外推多个循环，![](../graphics/usb_eqn00494.gif)。新损伤状态，![](../graphics/usb_eqn00504.gif)，由下式给出

![](../graphics/usb_eqn00505.gif)

其中![](../graphics/usb_eqn00506.gif)是与积分点相关的特征长度，![](../graphics/usb_eqn00507.gif)和![](../graphics/usb_eqn00508.gif)是材料常数（请参见["低循环疲劳中延性材料的损伤演化，" 第24.4.3节"](pt05ch24s04abm49.md)，以获取更多信息）。

您指定损伤外推的最小（![](../graphics/usb_eqn00509.gif)）和最大（![](../graphics/usb_eqn00510.gif)循环次数。默认值分别为100和1000。
...
如果在稳定循环结束时富集单元中任何裂纹尖端满足裂纹生长萌生准则，![](../graphics/usb_eqn00498.gif)，Abaqus/Standard通过在裂纹尖端前方断裂至少一个富集单元，将裂纹长度，![](../graphics/usb_eqn00514.gif)，从当前循环前向在外推多个循环，![](../graphics/usb_eqn00494.gif)，到![](../graphics/usb_eqn00515.gif)。给定材料常数![](../graphics/usb_eqn00516.gif)和![](../graphics/usb_eqn00517.gif)（如["使用扩展有限元方法将不连续性建模为富集特征，" 第10.7.1节"](pt04ch10s07at36.md)中所定义），结合裂纹尖前方富集单元的已知单元长度和可能的传播方向，![](../graphics/usb_eqn00518.gif)，计算裂纹尖端前方每个富集单元失效所需的循环次数为，![](../graphics/usb_eqn00519.gif)，其中![](../graphics/usb_eqn00520.gif)表示![](../graphics/usb_eqn00520.gif)-th裂纹尖前方的富集单元。分析设置在载荷循环稳定后每个增量至少将裂纹推进一个富集单元。具有最少循环数的单元被识别为断裂，其![](../graphics/usb_eqn00521.gif)表示裂纹生长其单元长度所需的循环数，![](../graphics/usb_eqn00522.gif)。最关键的单元在稳定循环结束时具有零约束和裂纹表面零刚度完全断裂。当富集单元断裂时，载荷被重新分配，必须为下一个循环计算裂纹尖前方富集单元的新相对断裂能释放率。此能力允许在每个稳定循环后至少断裂裂纹尖前方的一个富集单元，并精确计算导致疲劳裂纹沿该长度生长所需的循环次数。
...
如果在稳定循环结束时界面中任何裂纹尖端满足分层生长萌生准则，![](../graphics/usb_eqn00498.gif)，Abaqus/Standard通过在界面释放至少一个单元，将裂纹长度，![](../graphics/usb_eqn00514.gif)，从当前循环前向在外推多个循环，![](../graphics/usb_eqn00494.gif)，到![](../graphics/usb_eqn00515.gif)。给定材料常数![](../graphics/usb_eqn00516.gif)和![](../graphics/usb_eqn00517.gif)（如["裂纹扩展分析，" 第11.4.3节中的"低循环疲劳准则"](pt04ch11s04aus69.md#usb-anl-acrackpropagation-fatigue)中所定义），结合裂纹尖界面单元的已知节点间距，![](../graphics/usb_eqn00518.gif)，计算裂纹尖端处每个界面单元失效所需的循环次数为，![](../graphics/usb_eqn00519.gif)，其中*j*表示*j*th裂纹尖端的节点。分析设置在载荷循环稳定后每个增量至少释放一个界面单元。具有最少循环数的单元被识别为释放，其![](../graphics/usb_eqn00521.gif)表示裂纹生长其单元长度所需的循环数，![](../graphics/usb_eqn00522.gif)。最关键的单元在稳定循环结束时具有零约束和零刚度完全释放。当界面单元被释放时，载荷被重新分配，必须为下一个循环计算裂纹尖界面单元的新相对断裂能释放率。此能力允许在每个稳定循环后释放裂纹尖处至少一个界面单元，并精确计算导致疲劳裂纹沿该长度生长所需的循环次数。
