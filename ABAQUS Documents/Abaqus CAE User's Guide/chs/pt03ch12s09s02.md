...
5. 打开**使用与温度相关的数据**来定义与温度相关的数据。标有 **Temp** 的列出现在 **Data** 表中。
6. 单击**字段变量数量**字段右侧的箭头，以增加或减少数据所依赖的字段变量的数量。
7. 在 **数据** 表中输入以下数据： **摩擦角** *p*--平面中高围压下的摩擦角![](../graphics/usi_eqn00226.gif)。输入以度为单位的值。 **膨胀角** *p*--![](../graphics/usi_eqn00228.gif)平面中高围压下的膨胀角![](../graphics/usi_eqn00170.gif)。输入以度为单位的值。 **温度** 温度。 **字段 *n*** 预定义的字段变量。您可能需要展开对话框才能查看 **Data** 表中的所有列。有关如何输入数据的详细信息，请参阅["Entering tabular data," Section 3.2.7](pt01ch03s02s07.md)。
...
15. 在**数据**表中输入以下数据：**拉伸截止应力** 单轴拉伸时的屈服应力，![](../graphics/usi_eqn00229.gif)![](../graphics/usi_eqn00163.gif)。 **拉伸塑性应变** 相应的塑性应变。 （输入的第一个表格值必须始终为零。） **温度** 温度。 **字段 *n*** 预定义的字段变量。您可能需要展开对话框才能查看 **Data** 表中的所有列。有关如何输入数据的详细信息，请参阅["Entering tabular data," Section 3.2.7](pt01ch03s02s07.md)。
